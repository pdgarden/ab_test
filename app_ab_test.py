...
- There is 90% chances that group A's conversion rate minus groupb B's conversion high is higher than {np.percentile(diff, 10):.1%}
- There is 95% chances that group A's conversion rate minus groupb B's conversion high is higher than {np.percentile(diff, 5):.1%}
- There is 99% chances that group A's conversion rate minus groupb B's conversion high is higher than {np.percentile(diff, 1):.1%}
"""

st.markdown(summary)
st.subheader("Conversion rates by centile")
st.dataframe(df_repartitions)
